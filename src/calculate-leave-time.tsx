...
import { calculateLeaveTime, calculateRemainingTime, formatTimeString } from "./lib/time-utils";
...
export default function Command() {
  ...
  const remaining = leaveTime ? calculateRemainingTime(leaveTime, todayStart) : null;
      ...
      {todayStart && leaveTime && remaining && (
        <List.Section title={t.todaySection}>
          <List.Item
            ...
            subtitle={remaining.isPast ? t.overtime(remaining.hours, remaining.minutes) : t.remaining(remaining.hours, remaining.minutes)}
            icon={{ source: Icon.Clock, tintColor: remaining.isPast ? Color.Orange : Color.Blue }}
            accessories={[
              { tag: { value: todayStart, color: Color.SecondaryText } },
              { tag: { value: t.workBreakTag(workHours, breakMins), color: Color.SecondaryText } },
            ]}
            actions={
              <ActionPanel>
                <Action.CopyToClipboard title="Copy" content={leaveTime} />
                <Action title={t.reset} icon={Icon.Trash} style={Action.Style.Destructive} onAction={handleClear} />
              </ActionPanel>
            }
          />
        </List.Section>
      ...
      {customTime && !START_TIMES.includes(customTime) && (
        <List.Section title={t.customTimeSection}>
          <List.Item
            ...
            actions={
              <ActionPanel>
                <Action title={t.select} icon={Icon.Check} onAction={() => handleSelect(customTime)} />
              </ActionPanel>
            }
          />
        </List.Section>
        ...
        {filteredTimes.map((time) => {
          ...
          return (
            <List.Item
              ...
              actions={
                <ActionPanel>
                  <Action title={t.select} icon={Icon.Check} onAction={() => handleSelect(time)} />
                  <Action.CopyToClipboard title={t.copyLeaveTime} content={leave} />
                </ActionPanel>
              }
            />
          );
